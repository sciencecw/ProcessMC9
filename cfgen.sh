...
#for fullpath in /store/user/abelloni/EmJetAnalysis/Analysis-74xAODSIM_80xNTUPLES-v1/*/
  do folder=${fullpath%/} 
  folder=${folder##*/}
  configpath=configs/model_80Xb/${folder}_80X_config.txt
  parmspath=configs/modelparms_80Xb/parms_80X_${folder}.txt
  cp configs/parms_template.txt ${parmspath}
  fnum=`ls -1 ${fullpath}/*/*/*/*/ntuple_*.root | wc -l`
  echo ${folder} $fnum
  echo ${folder}>>configs/models_80Xb.txt
